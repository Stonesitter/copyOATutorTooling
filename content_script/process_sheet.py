...
def process_sheet(spreadsheet_key, sheet_name, default_path, is_local, latex, verbosity=False, course_name="", mode="full"):
    ...
    if meta:
        ...
        # If the tag "fixedProblemOrder" is present in the Meta column,
        # set the order of problems according to their appearance in the sheet.
        if "fixedProblemOrder" in meta_dict:
            problem_order = []
            for _, row in df.iterrows():
                row_type = row.get("Row Type", "")
                if isinstance(row_type, str) and row_type.strip().lower() == "problem":
            ...
            if problem_order:
                meta_dict["fixedProblemOrder"] = problem_order
            else:
                meta_dict.pop("fixedProblemOrder", None)
